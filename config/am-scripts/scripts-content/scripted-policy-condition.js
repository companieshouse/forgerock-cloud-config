var _scriptName = 'SCRIPTED POLICY CONDITION';
_log('Starting');
...
if (validateAndInitializeParameters()) {
  ...
  _log('Country retrieved from user\'s address: ' + countryFromUserAddress);
  var countryFromUserIP = getCountryFromUserIP();
  _log('Country retrieved from user\'s IP: ' + countryFromUserIP);
  var countryFromResourceURI = getCountryFromResourceURI();
  _log('Country retrieved from resource URI: ' + countryFromResourceURI);

  if (countryFromUserAddress === countryFromUserIP && countryFromUserAddress === countryFromResourceURI) {
    _log('Authorization Succeeded');
  ...
  _log('Required parameters not found. Authorization Failed.');
  authorized = false;
}
...
function validateAndInitializeParameters () {
  ...
  _log('Resource host: ' + resourceHost);

  return true;
}

function logResponse (response) {
  _log('User REST Call. Status: ' + response.getStatus() + ', Body: ' + response.getEntity().getString());
}
