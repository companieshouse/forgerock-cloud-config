...
function fetchActionParameter () {
  ...
  _log('action parameter not found in request, or different from \'changeUpdates\' or \'changeMarketing\'');
  return false;
}
...
try {
  ...
  if (!paramsResponse) {
    ...
    if (callbacks.isEmpty()) {
      ...
      _log('confirm remove: ' + confirmIndex);

      if (confirmIndex === ConfirmIndex.SUBMIT) {

        _log('email updates selection: ' + selection);
        var updateResponse;
