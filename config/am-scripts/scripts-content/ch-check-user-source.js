...
function buildUpdateUserPayload (ewfUserResult, existingLegacyPwd) {
  ...
  if (String(ewfUserResult.data.frIndexedString2) === String(existingLegacyPwd)) {
    _log('UPDATE USER: password unchanged', 'MESSAGE');
    requestBodyJson = [
    ...
    _log('UPDATE USER: password has changed', 'MESSAGE');
    requestBodyJson = [
      createPatchItem('mail', ewfUserResult.data.mail),
      createPatchItem('userName', ewfUserResult.data.mail),
      createPatchItem('sn', ewfUserResult.data.mail),
...
function createOrUpdateUser (accessToken, email) {
  try {
    ...
    if (!idmUserResult.success) {
      _log('[SYNC USER] User NOT found in IDM with email ' + email, 'MESSAGE');
      //search user in EWF by email
      ewfUserResult = fetchUserFromEWFByEmail(email);
      //the user exists in EWF
      if (ewfUserResult.success) {
        _log('[SYNC USER] User found in EWF for email ' + email + ' - Creating user in IDM', 'MESSAGE');
        ...
        if (idmUserWithSameParentUsername.success) {
          _log('[SYNC USER] User found in IDM with parent_username ' + ewfUserResult.data.frIndexedString1, 'MESSAGE');
          ...
          _log('[SYNC USER] User NOT found in IDM with parent_username ' + ewfUserResult.data.frIndexedString1, 'MESSAGE');
          operation = 'CREATE';
        ...
        _log('[SYNC USER] User not found in EWF or IDM for email ' + email, 'MESSAGE');
        return {
          success: false,
      ...
      _log('[SYNC USER] User exists in IDM for email ' + email, 'MESSAGE');
      //the IDM user has a PARENT_USERNAME set
      if (idmUserResult.userData.frIndexedString1) {
        _log('[SYNC USER] User has PARENT_USERNAME set: ' + idmUserResult.userData.frIndexedString1, 'MESSAGE');
        ...
        if (!ewfUserResult.success) {
          ...
          _log('[SYNC USER] User not found in EWF for PARENT_USERNAME ' + idmUserResult.userData.frIndexedString1, 'MESSAGE');
          ewfUserResult = fetchUserFromEWFByEmail(email);
          if (!ewfUserResult.success) {
            _log('[SYNC USER] User not found in EWF for email');
            return false;
          }
        }
      } else {
        _log('[SYNC USER] User does not have PARENT_USERNAME set: ' + email, 'MESSAGE');
        //search user in EWF by email
        ewfUserResult = fetchUserFromEWFByEmail(email);
        if (!ewfUserResult.success) {
          _log('[SYNC USER] User not found in EWF');
          return false;
        }
      }

      _log('[SYNC USER] Updating the user in fIDC: ' + email, 'MESSAGE');
      //update user in FIDC
    ...
    var response = httpClient.send(request).get();
    _log('[SYNC USER] IDM response: ' + response.getStatus().getCode());

    if (response.getStatus().getCode() === 201 || response.getStatus().getCode() === 200) {

      if (response.getStatus().getCode() === 200){
        _log('[SYNC USER] User Updated from EWF');
      }
      if (response.getStatus().getCode() === 201){
        _log('[SYNC USER] User Created from EWF');
      }
      ...
      _log('[SYNC USER] Error during user creation/update of user');
      return {
        success: false,
        operation: operation,
    ...
    _log('[SYNC USER] TOPLEVEL Exception occurred during user creation/update of user - ' + e);
  }
}
...
function fetchUserFromEWFBySearchTerm (accessToken, searchTerm) {
  ...
  try {
    ...
    request.getHeaders().add('Content-Type', 'application/json');
    request.getHeaders().add('Accept-API-Version', 'resource=1.0');
    _log('[SYNC USER FROM EWF] Request to Oracle connector (users): '+ SYSTEM_WEBFILING_USER + searchTerm, 'MESSAGE')
    var httpResp = httpClient.send(request).get();
    _log('[SYNC USER FROM EWF] Response from EWF User connector: Status ' + httpResp.getStatus().getCode(), 'MESSAGE');
    
    if (httpResp.getStatus().getCode() !== 200) {
      _log('[SYNC USER FROM EWF] Error while fetching EWF User');
      _log('[SYNC USER FROM EWF] Error while fetching EWF User: ' + httpResp.getEntity().getString(), 'MESSAGE');
      return {
        success: false,
    ...
    if (response.resultCount === 1) {
      _log('[SYNC USER FROM EWF] User found in EWF database', 'MESSAGE');
      ...
      _log('[SYNC USER FROM EWF] No Users found in EWF database');
      _log('[SYNC USER FROM EWF] No Users found in EWF database for searchTerm ' + searchTerm, 'MESSAGE');
      return {
        success: false,
...
try {
  ...
  if (!accessToken || !username) {
    _log('[SYNC USER] Access token or user email not in shared state', 'MESSAGE');
    ...
    if (!result) {
      _log('[SYNC USER] User creation/update from source failed - proceeding to login with existing IDM credentials', 'MESSAGE');
      action = fr.Action.goTo(NodeOutcome.ERROR).build();
    } else {
      _log('[SYNC USER] User successfully created/updated from EWF source', 'MESSAGE');
